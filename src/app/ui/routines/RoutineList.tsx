	...
	return (
		<>
			{routines.map(routine => (
				<Card key={routine.id} variant="outlined" 
				// TODO revisar este código duplicado en RoutineGriupCard
					...
					<CardActionArea LinkComponent={Link} href={`/routines/${routine.id}`}>
						<CardHeader title={routine.name} titleTypographyProps={{variant: 'subtitle1'}}
							subheader={`Created: ${  routine.createdAt}`}
						 avatar={
								<Avatar>
									{routine.name.split(' ').map(word => word[0]).join('')}
								</Avatar>
							}/>
